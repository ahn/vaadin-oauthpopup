...
import org.vaadin.addon.oauthpopup.oauth10a.OAuth10aPopupOpenerImpl;
...
public class DemoLayout extends VerticalLayout {
  ...
  private void addTwitterButtons() {
    final ApiInfo api = readClientSecrets("/client_secret.twitter.json",
                                          "Twitter",
                                          TwitterApi.instance(),
    ...
    final TwitterButton button = new TwitterButton(api.apiKey, api.apiSecret);
    addButton(api, button);
    final NativeButton b = new NativeButton("Another Twitter Auth Button");

    final OAuth10aPopupOpenerImpl opener = new OAuth10aPopupOpenerImpl(TwitterApi.instance(), api.apiKey, api.apiSecret);
    opener.extend(b);
  ...
  private void addLinkedInButton() {
    final ApiInfo api = readClientSecrets("/client_secret.linkedin.json",
                                          "LinkedIn",
    ...
    final LinkedInButton button = new LinkedInButton(api.apiKey, api.apiSecret);
    addButton(api, button);
  }

  private void addGitHubButton() {
    final ApiInfo api = readClientSecrets("/client_secret.github.json",
                                          "GitHub",
                                          GitHubApi.instance(),
    ...
    final GitHubButton button = new GitHubButton(api.apiKey, api.apiSecret);
    addButton(api, button);
  }

  private void addGoogleButton() {
    final ApiInfo api = readClientSecrets("/client_secret.google.json",
                                          "Google",
    ...
    final GoogleButton button = new GoogleButton(api.apiKey, api.apiSecret, "https://www.googleapis.com/auth/plus.login");
    addButton(api, button);
  }
  ...
  private ApiInfo readClientSecrets(String resourcePath, String name, Object scribeApi, String getEndpoint) {
    ...
    if (getClass().getResource(resourcePath) != null) {
      try {
        final JsonNode web = new ObjectMapper().readTree(getClass().getResourceAsStream(resourcePath));
        if (web != null) {
  ...
  private void addButton(final ApiInfo service, OAuthPopupButton button) {
    ...
    final HorizontalLayout hola = new HorizontalLayout();
  ...
  private class Listener implements OAuthListener {
    ...
    @Override
    public void authSuccessful(final Token token) {
      final Label l = new Label("Authorized.");
      hola.addComponent(l);
      hola.setComponentAlignment(l, Alignment.MIDDLE_CENTER);

      final Button testButton = new Button("Test " + service.name + " API");
      ...
      testButton.addClickListener(new ClickListener() {
        ...
        public void buttonClick(ClickEvent event) {
          final GetTestComponent get = new GetTestComponent(service, token);
          final Window w = new Window(service.name, get);
          w.center();
          w.setWidth("75%");
          w.setHeight("75%");
    ...
    @Override
    public void authDenied(String reason) {
      final Label l = new Label("Auth failed.");
      hola.addComponent(l);
      hola.setComponentAlignment(l, Alignment.MIDDLE_CENTER);
    }
  }
