...
public abstract class OAuthCallbackRequestHandler<S extends OAuthService, T extends BaseApi<S>> implements RequestHandler {
   ...
   *
   * @param requestToken
   *        may be null (in case of OAuth2)
   * @param data
   */
  public OAuthCallbackRequestHandler(OAuthDataAbstract<S, T> data) {
    this.data = data;
  }
  ...
  public boolean handleRequest(VaadinSession session, VaadinRequest request,
    ...
    if (!data.isCallbackForMe(request)) {
      // Debugger
      /*
       * Logger.getGlobal().log(Level.INFO, "Request isn't for me"); for (Map.Entry<String, String[]> e :
       * request.getParameterMap().entrySet()) { Logger.getGlobal().log(Level.INFO,
       * "\t"+e.getKey()+"="+URLDecoder.decode(e.getValue()[0], "UTF-8")); }
       */
      return false;
    }

    final String verifier = request.getParameter(data.getOAuthPopupConfig().getVerifierParameterName());
  ...
  public void cleanUpSession(final VaadinSession session) {
    session.access(() -> session.removeRequestHandler(OAuthCallbackRequestHandler.this));
  }
}
